...
// ------------------------------------------------------------------ chart
// SVG درون‌خطی، بدون کتابخانه. دلیلش همان قاعده‌ی پروژه است: هسته
// وابستگی خارجی ندارد و داشبورد هم بدون build کار می‌کند.
...
/** نگاشت مقدار به مختصات، با محافظت از تقسیم بر صفر (سری تخت). */
function scaler(min, max, from, to) {
  const span = max - min;
  if (!span) return () => (from + to) / 2;   // سری کاملاً تخت → وسط
  return (v) => to - ((v - min) / span) * (to - from);
}

function drawChart(points, opts = {}) {
  const W = 720, H = 220, PAD_L = 52, PAD_R = 12, PAD_T = 12, PAD_B = 26;
  const root = svg("svg", {
    class: "chart-svg",
    viewBox: `0 0 ${W} ${H}`,
  ...
  const values = points.map((p) => p.close).filter((v) => v > 0);
  if (values.length < 2) {
    root.append(svg("text", { x: W / 2, y: H / 2, class: "chart-axis",
      "text-anchor": "middle" }));
    root.lastChild.textContent = "داده کافی برای نمودار نیست.";
    return root;
  }

  // محدوده با کمی حاشیه، و خط‌های مرجع هم باید داخل کادر بمانند
  const refs = (opts.refs || []).map((r) => r.value).filter((v) => v > 0);
  ...
  const y = scaler(lo, hi, PAD_T, H - PAD_B);

  // شبکه و برچسب محور عمودی
  for (let i = 0; i <= 3; i++) {
    const v = lo + ((hi - lo) * i) / 3;
    const gy = y(v);
    root.append(svg("line", { class: "chart-grid", x1: PAD_L, x2: W - PAD_R, y1: gy, y2: gy }));
    const label = svg("text", { class: "chart-axis", x: PAD_L - 6, y: gy + 3, "text-anchor": "end" });
    label.textContent = fmt(v);
    root.append(label);
  }

  const cls = opts.premium ? " premium" : "";
  const line = points.map((p, i) => `${i ? "L" : "M"}${x(i).toFixed(1)},${y(p.close).toFixed(1)}`).join(" ");
  root.append(svg("path", {
    class: "chart-area" + cls,
    d: `${line} L${x(points.length - 1).toFixed(1)},${H - PAD_B} L${x(0).toFixed(1)},${H - PAD_B} Z`,
  }));
  root.append(svg("path", { class: "chart-line" + cls, d: line }));

  // روزهای بدون معامله — قیمتشان تکرار دیروز است، نه حرکت واقعی
  points.forEach((p, i) => {
    if (p.traded === false) {
      root.append(svg("circle", { class: "chart-untraded", cx: x(i), cy: y(p.close), r: 2.5 }));
    }
  });

  // خطوط مرجع سیگنال (قیمت اعمال، قیمت ورود)
  (opts.refs || []).forEach((ref) => {
    if (!(ref.value > 0)) return;
    const ry = y(ref.value);
    root.append(svg("line", {
      class: "chart-ref " + ref.kind, x1: PAD_L, x2: W - PAD_R, y1: ry, y2: ry,
    }));
    const t = svg("text", { class: "chart-axis", x: W - PAD_R - 2, y: ry - 3, "text-anchor": "end" });
    t.textContent = `${ref.label} ${fmt(ref.value)}`;
    root.append(t);
  });

  // محور افقی: فقط اول و آخر — بیشتر از این روی موبایل خوانا نیست
  [[0, "start"], [points.length - 1, "end"]].forEach(([i, anchor]) => {
    const t = svg("text", { class: "chart-axis", x: x(i), y: H - 8, "text-anchor": anchor });
...
/** نمودار یک سیگنال را می‌سازد و داخل کارت می‌گذارد. */
async function renderSignalChart(box, s) {
  box.innerHTML = '<p class="empty">در حال خواندن نمودار…</p>';
  try {
    const params = new URLSearchParams({ underlying: s.underlying || "", days: "120" });
    if (s.symbol) params.set("symbol", s.symbol);
    ...
    const views = [];
    if (d.base && d.base.length) views.push("base");
    if (d.premium_available) views.push("premium");
    if (!views.length) {
      box.append(el("p", "empty", "داده‌ای برای نمودار نبود."));
      return;
    }

    const tabs = el("div", "chart-tabs");
    const canvas = el("div", "chart-wrap");
    ...
    const show = (view) => {
      ...
      canvas.innerHTML = "";
      if (view === "base") {
        canvas.append(drawChart(d.base, {
          refs: [
            { value: s.strike, kind: "strike", label: "اعمال" },
            { value: s.underlying_price, kind: "entry", label: "ورود" },
          ],
        }));
        note.textContent = `قیمت پایه ${d.underlying} — ${fmt(d.base.length)} روز`;
      } else {
        const untraded = d.premium.filter((p) => !p.traded).length;
        canvas.append(drawChart(d.premium, {
          premium: true,
          refs: [{ value: s.suggested_price, kind: "entry", label: "پرمیوم سیگنال" }],
        }));
        note.textContent =
          `پرمیوم واقعی ${s.symbol} — ${fmt(d.premium.length)} روز` +
          (untraded ? ` · ${fmt(untraded)} روز بدون معامله (نقطه قرمز)` : "");
      }
    };

    const labels = { base: "نماد پایه", premium: "پرمیوم قرارداد" };
    views.forEach((v) => {
      const b = el("button", "chart-tab", labels[v]);
      b.dataset.view = v;
      b.addEventListener("click", (e) => { e.stopPropagation(); show(v); });
      tabs.append(b);
    });
    if (!d.premium_available) {
      const b = el("button", "chart-tab", labels.premium);
      b.disabled = true;
      b.title = "این سیگنال کد یکتای قرارداد را ذخیره نکرده (سیگنال قدیمی).";
      tabs.append(b);
    }

    box.append(tabs, canvas, note);
    box.append(legend([
      ["var(--info)", "قیمت پایه"],
      ["var(--garnet-soft)", "پرمیوم"],
      ["var(--warn)", "قیمت اعمال"],
      ["var(--put)", "روز بدون معامله"],
    ]));
    show(views[0]);
  } catch (err) {
    box.innerHTML = "";
...
function signalCard(s) {
  ...
  head.append(el("span", "sig-toggle", "▾ نمودار"));
  card.append(head);
  ...
  // کلیک روی کارت ⇒ نمودار. تنبل بارگذاری می‌شود: کشیدنِ نمودارِ همه‌ی
  // سیگنال‌ها یعنی ده‌ها درخواست شبکه برای چیزی که کاربر ندیده.
  const chartBox = el("div", "sig-chart");
  ...
  const toggle = () => {
    const opening = chartBox.hidden;
    chartBox.hidden = !opening;
    card.classList.toggle("open", opening);
    card.setAttribute("aria-expanded", String(opening));
    card.querySelector(".sig-toggle").textContent = opening ? "▴ بستن" : "▾ نمودار";
    if (opening && !chartBox.dataset.loaded) {
      chartBox.dataset.loaded = "1";
      renderSignalChart(chartBox, s);
    }
  };

  card.addEventListener("click", (e) => {
    // کلیک داخل خودِ نمودار (تب‌ها) نباید کارت را ببندد
    if (e.target.closest(".sig-chart")) return;
    toggle();
  });
